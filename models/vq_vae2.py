...
class VQVAE2(nn.Module):
    ...
    def forward(self, input, rel_pose):
        ...
        if self.bPoseCondition:
            ...
            quant_b = torch.cat((quant_b, rel_pose_emb2), dim=1)
        dec = self.decode(quant_t, quant_b)
        dummy = 0

        return dec, diff, dummy

    def loss(self, input, recon, latent_loss, dummy):
        recon_loss = F.mse_loss(recon, input)
        latent_loss = latent_loss.mean()
        loss = recon_loss + self.beta * latent_loss

        return loss, recon_loss.detach(), latent_loss.detach()
